...
import { nowPlaylist, NowPlaylistActions, NowPlaylistTypes } from './index';
import * as NowPlaylist from './now-playlist.actions';
...
describe('The Now Playlist Reducer', () => {
  it('should return current state when no valid actions have been made', () => {
    ...
    const actual = nowPlaylist(state, { type: 'INVALID_ACTION' });
  ...
  it('should select the chosen video', () => {
    ...
    const actual = nowPlaylist(
      <any>state,
      new NowPlaylist.SelectVideo(<any>YoutubeMediaItemsMock[0])
    );
  ...
  it('should queue the selected video to the list', () => {
    ...
    const actual = nowPlaylist(
      <any>state,
      new NowPlaylist.QueueVideo(<any>newVideo)
    );
