import { player, IAppPlayer, AppPlayerActionTypes } from './app-player.reducer';
...
describe('The App Player reducer', () => {
  ...
  it('should set the new media id by the new PLAYED youtube media item', () => {
    const state = { ...mockedState };
    const actual = player(state, { type: AppPlayerActionTypes.PLAY, payload: YoutubeMediaMock });
  ...
  it('should toggle visibility of the player', () => {
    ...
    const actual = player(state, { type: AppPlayerActionTypes.TOGGLE_PLAYER, payload: true });
    const expected = state;
  ...
  it('should change the state of the player', () => {
    ...
    const actual = player(state, { type: AppPlayerActionTypes.UPDATE_STATE, payload: 1 });
